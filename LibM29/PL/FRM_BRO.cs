        ...
        private void btnadd_Click_1(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (ID == 0)
                {
                    ...
                    BLSEEL.insert(Convert.ToString(dataGridView2.CurrentRow.Cells[1].Value), Convert.ToString(dataGridView1.CurrentRow.Cells[1].Value), Convert.ToString(txt_date.Value), Convert.ToString(txt_date2.Value), Convert.ToInt32(txt_title.Text));
                    PL.FRM_DADD Fadd = new FRM_DADD();
                    Fadd.Show();
                    this.Close();
                }
                ...
                {
                    BL.CLS_BOR BLSEEL = new BL.CLS_BOR();
                    BLSEEL.insert(Convert.ToString(dataGridView2.CurrentRow.Cells[1].Value), Convert.ToString(dataGridView1.CurrentRow.Cells[1].Value), Convert.ToString(txt_date.Value), Convert.ToString(txt_date2.Value), Convert.ToInt32(txt_title.Text), ID);
                    PL.FRM_DEDT fEdIT = new FRM_DEDT();
                    fEdIT.Show();
                    this.Close();
                }
